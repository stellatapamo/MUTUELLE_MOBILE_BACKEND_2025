...
public class EpargneController {
        ...
        // ========================================================================================
        @PostMapping
        @Operation(summary = "Créer une transaction d'épargne", description = "Permet de créer une transaction d'épargne (CREDIT ou DEBIT) pour un membre.")
        @ApiResponse(responseCode = "200", description = "Transaction créée avec succès", content = @Content(schema = @Schema(implementation = Transaction.class)))
        @ApiResponse(responseCode = "400", description = "Paramètres invalides")
        public ResponseEntity<ApiResponseDto<TransactionEpargneDto>> transactionEpargne(
                        @Parameter(description = "ID du membre concerné", example = "1") @RequestParam Long memberId,

                        @Parameter(description = "ID de la session", example = "3") @RequestParam Long sessionId,

                        @Parameter(description = "Montant de l'épargne", example = "5000") @RequestParam BigDecimal amount,

                        @Parameter(description = "Direction de la transaction : CREDIT ou DEBIT", example = "CREDIT") @RequestParam String transactionDirection) {
                ...
                try {
                        ...
                        return ResponseEntity.badRequest().body(
                                        ApiResponseDto.error(
                                                        "TransactionDirection invalide : utilisez CREDIT ou DEBIT"));
                }

                Transaction transaction = epargneService.processEpargne(memberId, sessionId, amount, direction);
